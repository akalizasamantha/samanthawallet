...
def wallet(request): 
    if request.method == 'POST':
        ...
        if check_existing:
            ...
            finduser = Account.objects.get(email=username)
    ...
    return render(request, 'core/sendmoney.html')
...
def registerUser(request):
    ...
    if request.method == 'POST':
        ...
        if regpassword1 == regpassword2:
            ...
            check_user = Account.objects.filter(email=regemail).exists()
            if check_user:
                messages.warning(request, f"user exists")
                return redirect('register')

            else:
                 saveuser = Account.objects.create_user(user_name=regusername, email=regemail, first_name='',  password=regpassword1)
            return redirect('login')
        else:
